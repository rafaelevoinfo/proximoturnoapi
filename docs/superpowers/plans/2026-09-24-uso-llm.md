...
- **Prático:** se a instância da API do dono do repositório estiver rodando, ela tranca `Src/bin` e o build falha ao copiar o `.exe`. Compile e teste com `--artifacts-path "<pasta temporária>/art"`, que redireciona a saída: `dotnet test Tests/Tests.csproj --artifacts-path <pasta>/art --filter <...>`.
...
Run: `dotnet build Src/ProximoTurnoApi.csproj --artifacts-path <pasta>/art`
Expected: 0 erros.
...
Se aparecer foreign key para `JOGO_LINK`, a convenção do EF inferiu relação pelo nome da propriedade: acrescente `b.Ignore(...)` não resolve — declare explicitamente no `ConfigureUsoLlm` que não há relação, removendo qualquer `HasOne` e mantendo `IdJogoLink` como `int?` puro, e gere a migração de novo.
...
Run: `dotnet test Tests/Tests.csproj --artifacts-path <pasta>/art --filter FullyQualifiedName~EscopoUsoLlmTests`
...
Run: `dotnet test Tests/Tests.csproj --artifacts-path <pasta>/art --filter FullyQualifiedName~RegistradorUsoLlmTests`
...
Run: `dotnet test Tests/Tests.csproj --artifacts-path <pasta>/art --filter FullyQualifiedName~PoliticaUsoLlmTests`
...
public sealed class PoliticaUsoLlm(ILogger<PoliticaUsoLlm> _logger,
    ...
    public override void Process(PipelineMessage mensagem, IReadOnlyList<PipelinePolicy> pipeline, int indice) {
        ...
        try {
            ...
            Entregar(mensagem, relogio, excecao, sincrono: true);
            throw;
        }

        Entregar(mensagem, relogio, null, sincrono: true);
    }
    ...
    private void Entregar(PipelineMessage mensagem, Stopwatch relogio, Exception? excecao, bool sincrono) {
...
Run: `dotnet test Tests/Tests.csproj --artifacts-path <pasta>/art --filter FullyQualifiedName~PoliticaUsoLlmTests`
Expected: 15 passed (13 `[Fact]` mais as 3 linhas do `[Theory]`, menos a contagem que o runner agrupa — o que importa é 0 failed).
...
Run: `dotnet test Tests/Tests.csproj --artifacts-path <pasta>/art --filter FullyQualifiedName~FabricaOpenRouterTests`
...
```csharp
    [Fact]
    public async Task Indexacao_AbreOEscopoDeAlvoAntesDeChamarLlm() {
        await Executar();

        Assert.Equal(1, _extrator.AlvoVisto?.IdJogo);
        Assert.Equal(7, _extrator.AlvoVisto?.IdJogoLink);
        Assert.Equal("Jogo de Teste > Manual", _extrator.AlvoVisto?.Alvo);
        Assert.Null(EscopoUsoLlm.Atual);
    }
```

Ajuste os três valores esperados para o que o `EstadoLinkManual` do `SincronizarManualTests` já usa (ids e nomes do fixture existente): leia o helper que monta o estado no topo do arquivo e use `estado.IdJogo`, `estado.IdJogoLink` e `$"{estado.NomeJogo} > {estado.TituloLink}"`, que é o que `ContextoManual.Prefixo` produz.

- [ ] **Step 2: Rodar para ver falhar**

Run: `dotnet test Tests/Tests.csproj --artifacts-path <pasta>/art --filter FullyQualifiedName~SincronizarManualTests`
...
Run: `dotnet test Tests/Tests.csproj --artifacts-path <pasta>/art`
Expected: 0 failed. A suíte tinha 316 testes antes desta feature; agora deve ter ~344.
